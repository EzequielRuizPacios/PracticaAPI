...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        const parsed: CatFact[] = JSON.parse(raw);
        setHistory(parsed);
      }
    } catch (e) {
  ...
  const addToHistory = (text: string, length: number) => {
    ...
    if (history.length > 0 && history[0].fact === text) return;
  ...
  const fetchCatFact = async () => {
    try {
      ...
      const text: string = data.fact ?? "Sin fact";
      setFact(text);
    } catch (error) {
  ...
  const clearHistory = () => {
    if (!confirm("¿Seguro de querer borrar todo el historial?")) return;
    setHistory([]);
  };
